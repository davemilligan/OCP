...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		// If the type is specified then the parenthesis are needed.
		...
		// Dont even need the parameter brackets if there is only one parameter
		// and type is not specified.
		// When using braces for the body, all statements must end with a semi
		// colon, else a compiler error.
		printIfHopper(animals, a -> {
			System.out.format("In lambda without brackets checking %s %n", a);
			return a.canHop();
		});

		// In fact if there is only one parameter we can just pass a reference
		// to the method.
		...
		// Or just pass a reference to any method that takes an Animal
		// parameter, and returns void.
		consumeAnimal(animals, Main::printAnimal);

		// Or maybe even, the System.out.println function can take an Animal,
		// and it returns void, what it does is immaterial.
		consumeAnimal(animals, System.out::println);

		// But lastly, there is already a Functional Interface that fulfils the
		// contract we defined in ConsumeAnimal,
		// The Consumer<T> interface, it has a method accept that accepts an
		// object T, and returns void...perfect.
		useConsumer(animals, System.out::println);

		// If it comes to it our CheckTrait is also redundant as there exist a
		// Functional Interface called Predicate<T>
		// that has a test(T t) method that returns boolean.
		predicateAnimal(animals, Animal::canHop);

		// In fact we can pass any method reference that fits the test(T t)
		// interface.
	 ...
	 *            mapped to the CheckTrait interface definition: boolean
	 *            check(Animal a); So for lambda a -> a.canHop(); 'a' is the
	 *            parameter to the method. 'a.canHop()' becomes the body.
	 */
